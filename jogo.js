...
function desenhar(){
    ...
    if(moduloLunar.posicao.y >= (canvas.height - 0.5 * moduloLunar.altura )){
        ...
        {
            return mostrarResultado("VOCÊ SE LASCOU KKKKKK", "red");
        }else{
            return mostrarResultado("AEEE, VOCÊ CONSEGUIU POUSAR!", "lime");
        }
    }
...
function atracaoGravitacional(){
    ...
    if(moduloLunar.motorLigado){
        moduloLunar.velocidade.y -= 0.0165 * Math.cos(moduloLunar.angulo);
        moduloLunar.velocidade.x += 0.0165 * Math.sin(moduloLunar.angulo);
    }
